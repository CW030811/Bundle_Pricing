        ...
        for i, mp in enumerate(sorted(inst_dir_n.glob('*.msgpack')), start=1):
            ...
            if n == 5:
                try:
                    ...
                    revenue_in = res_mb.get('objective', None)
                    revenue_in_eval = revenue_in
                    revenue_out = eval_mb_out_of_sample(v_kn, c_n, res_mb.get('bundle_prices', {}), res_mb.get('assortments')) if res_mb.get('bundle_prices') is not None else None
                ...
                out_json.write_text(json.dumps(res_mb, ensure_ascii=False, indent=2, default=lambda x: x.tolist() if hasattr(x,'tolist') else x), encoding='utf-8')
                rows.append({
                    "instance_id": f"n{n}_inst{i:03d}",
                    "seed": setup.get('seed',''),
                    "n": n,
