"""CPBSD instance generator (general utility).

NOTE: This file is NOT the generator used for the paper's random-valuation
experiments (Table 5 / Appendix A.4). In particular, its ``random_corr`` cost
ratio is ``rng.uniform(0.3, 0.9)``, whereas the paper uses ``r_i ~ Uniform(0, 1)``.
The paper / Table-5 experiments are produced by
``CORE_FIVE_DIRECT_RUN_SCRIPTS/04_compare_fcp_pruned_mb_bsp_cpbsdA_setups.py``
(and reproduced under ``random_valuation_zfix_repro_20260619/``), which use the
``Uniform(0, 1)`` ratio. Do not use this file to reproduce Table 5 random_corr
results.
"""
import argparse
import math
