"""
PCP Cutoff Sensitivity Experiment

Sensitivity test for Cutoff=0.5 in PCP strategy.
- Test dataset: 10 samples from each of m10n10, m20n10, m30n10 (30 total)
...
SAMPLES_PER_DATASET = 10  # PCP uses 10 per dataset (vs FCP's 30)
MB_SUBDIRS = ["test_m10n10_1e_3", "test_m20n10_1e_3", "test_m30n10_1e_3"]
OUTPUT_CSV = "cutoff_sensitivity_PCP_results.csv"
OUTPUT_REVENUE_PNG = "cutoff_sensitivity_PCP_revenue.png"
OUTPUT_METRICS_PNG = "cutoff_sensitivity_PCP_metrics.png"
OUTPUT_PARETO_PNG = "cutoff_sensitivity_PCP_pareto.png"

# Import from test_PCP
from test_PCP import EdgeScoringGCN, process_data, revenue_ratio, top_m_selection, generate_progressive_bundles


def load_test_samples():
    """Load 10 samples from each of m10n10, m20n10, m30n10."""
    samples = []
    for subdir in MB_SUBDIRS:
        ...
        for idx in indices:
            ...
            try:
                ...
                n, m, unit_cs, ship_cs, unit_us, Ns, opt_bundles, opt_prices, opt_rev, running_time, gap, stored_cs, stored_Rs = misc
                samples.append({
                    "dat": dat,
                    "misc": misc,
                    "dataset": subdir,
...
def selected_products_to_pred_assort(selected_products, n, m):
    """Convert selected_products (list of lists) to pred_assort (m, n) matrix."""
...
def run_experiment():
    print("=" * 60)
    print("PCP Cutoff Sensitivity Experiment")
    print("=" * 60)
    ...
    for cutoff in tqdm(CUTOFFS, desc="Cutoffs"):
        for i, s in enumerate(samples):
            ...
            sigmoid_output = 1.0 / (1.0 + np.exp(-logits_nm))  # (n, m)
            
            # PCP: top_m_selection with threshold=cutoff
            ...
            # Build pred_assort for metrics (from selected_products)
            pred_assort = selected_products_to_pred_assort(selected_products, n, m)

            n, segment_num, unit_cs, ship_cs, unit_us, Ns, opt_bundles, opt_prices, opt_rev, running_time, gap, stored_cs, stored_Rs = s["misc"]

            try:
                rev_ratio, milp_time = revenue_ratio(n, segment_num, unit_cs, ship_cs, unit_us, Ns, opt_rev, 
    ...
    ax1.set_title("PCP Cutoff Sensitivity: Revenue Ratio")
    ax1.legend()
    ...
    plt.tight_layout()
    plt.savefig("cutoff_sensitivity_PCP_revenue_by_dataset.png", dpi=300, bbox_inches="tight")
    plt.close()
    ...
    if np.any(valid_pts):
        ...
        ax4.set_title("PCP Cutoff: Revenue vs Time Trade-off (Pareto)")
