...
当前工作区默认参考 `run_cpbsd_baselines_v2.py` 产出的 `unified_log.json/csv`。以下字段说明与旧 `run_cpbsd_baselines.py` 基本同构，可继续作为统一口径参考：
...
- 线性化：`big_m`
- 追溯：`result_path`
- 范围声明：`experiment_scope`（当前常见值为 `smoke_subset_v2`）
