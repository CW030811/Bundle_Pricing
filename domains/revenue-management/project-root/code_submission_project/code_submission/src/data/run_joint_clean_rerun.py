"""
Clean re-run: Joint + baselines on old instances (seed=20260424).
N=5 and N=10, random_ind and random_corr.
All methods use the same instance file, same OOS samples.
"""
...
from solve_mb_bsp_on_cpbsd_v2 import (
    build_assortments, eval_bsp_policy, eval_mb_policy, json_default,
...
def get_assortments(n, cost, inst_name):
    if n <= 5:
        return build_assortments(n)
    fcp_file = EXP / f"fcp_random_cost_eval_n{n}_{cost}/results/{inst_name}__fcp_pruned_mb.json"
...
def main():
    ...
    setups = [
        (5, "random_ind"), (5, "random_corr"),
        (10, "random_ind"), (10, "random_corr"),
    ]
    TL = 600.0; GAP = 1e-2
    all_rows = []
    ...
    for n, cost in setups:
        ...
        for idx, ip in enumerate(paths):
            ...
            ass = get_assortments(n, cost, name)
    ...
    # --- Summary ---
    print(f"\n{'='*90}")
    print("AVERAGES BY SETUP")
    print(f"{'='*90}")

    by_setup = defaultdict(lambda: defaultdict(list))
    for r in all_rows:
        by_setup[(r["N"], r["cost"])][r["method"]].append(r)

    for (n_val, cost_val), md in sorted(by_setup.items()):
        print(f"\n### N={n_val} / {cost_val}")
    ...
    out = EXP / "joint_clean_rerun_n5_n10.json"
